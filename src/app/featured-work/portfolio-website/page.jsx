import Navbar from "@/components/Navbar";
import Link from "next/link";
import Footer from "@/components/Footer";

export default function PortfolioWebsitePage() {
  return (
    <main className="min-h-screen bg-[#0F0F0F] text-white">
      <Navbar />

      <section className="max-w-7xl mx-auto px-8 py-20">

        {/* Back */}
        <Link
          href="/featured-work"
          className="inline-flex items-center gap-2 text-[#6B7280] hover:text-amber-400 transition-colors duration-200 text-sm mb-14 group"
        >
          <span className="group-hover:-translate-x-1 transition-transform duration-200">←</span>
          Back to Featured Work
        </Link>

        {/* ── Hero ── */}
        <div className="mb-16">
          <div className="flex flex-wrap items-center gap-3 mb-6">
            <span className="px-4 py-1.5 rounded-full bg-purple-500/10 border border-purple-500/20 text-purple-400 font-semibold text-sm">
              Project
            </span>
            <span className="px-4 py-1.5 rounded-full bg-[#1C1C1C] border border-[#2A2A2A] text-[#6B7280] text-sm">
              2026 — Present
            </span>
            <span className="px-4 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-400 text-sm font-medium">
              Live ↗
            </span>
          </div>

          <h1 className="text-6xl md:text-8xl font-black text-white leading-none tracking-tight mb-8">
            Portfolio <span className="text-amber-400">Website</span>
          </h1>

          <p className="text-xl text-[#6B7280] max-w-2xl leading-relaxed">
            A modern portfolio website built to showcase my journey as a Computer Science student,
            featuring responsive design, smooth interactions, and a clean dark aesthetic.
          </p>
        </div>

        {/* ── Project Image Placeholder ── */}
        <div className="w-full h-72 md:h-96 bg-[#161616] border border-[#2A2A2A] rounded-3xl mb-16 flex items-center justify-center relative overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-purple-500/5 via-transparent to-amber-500/5"></div>
          <div className="text-center relative z-10">
            <p className="text-[#2A2A2A] text-6xl font-black">Preview</p>
            <p className="text-[#333] text-sm mt-2">Screenshot / Demo GIF</p>
          </div>
        </div>

        {/* ── Detail Grid ── */}
        <div className="grid md:grid-cols-3 gap-6 mb-16">

          {/* Tech Stack */}
          <div className="md:col-span-1 bg-[#161616] border border-[#2A2A2A] rounded-2xl p-6">
            <p className="text-xs text-[#6B7280] uppercase tracking-widest mb-4">Tech Stack</p>
            <div className="flex flex-wrap gap-2">
              {["Next.js", "React", "Tailwind CSS", "Node.js"].map((t) => (
                <span key={t} className="px-3 py-1.5 bg-[#1C1C1C] border border-[#2A2A2A] rounded-lg text-sm text-[#9CA3AF]">
                  {t}
                </span>
              ))}
            </div>
          </div>

          {/* Key Stats */}
          <div className="md:col-span-2 grid grid-cols-3 gap-4">
            {[
              { num: "100%", label: "Responsive" },
              { num: "Dark", label: "Craft UI" },
              { num: "2026", label: "Launched" },
            ].map(({ num, label }) => (
              <div key={label} className="bg-[#161616] border border-[#2A2A2A] rounded-2xl p-6 flex flex-col justify-between">
                <p className="text-4xl font-black text-white">{num}</p>
                <p className="text-xs text-[#6B7280] uppercase tracking-widest mt-2">{label}</p>
              </div>
            ))}
          </div>
        </div>

        {/* ── About the Project ── */}
        <div className="grid md:grid-cols-2 gap-8 mb-16">

          <div className="bg-[#161616] border border-[#2A2A2A] rounded-2xl p-8">
            <p className="text-xs text-[#6B7280] uppercase tracking-widest mb-4">About</p>
            <h3 className="text-xl font-bold text-white mb-4">What is this project?</h3>
            <p className="text-[#6B7280] leading-relaxed">
              This is my personal portfolio website, designed and built from scratch to showcase
              my projects, academic journey, certifications, and skills. It's fully responsive,
              accessible, and crafted with a dark-first design philosophy using the Dark Craft aesthetic.
            </p>
          </div>

          <div className="bg-[#161616] border border-[#2A2A2A] rounded-2xl p-8">
            <p className="text-xs text-[#6B7280] uppercase tracking-widest mb-4">Learnings</p>
            <h3 className="text-xl font-bold text-white mb-4">What I built with it</h3>
            <ul className="space-y-3">
              {[
                "Next.js App Router & file-based routing",
                "Responsive layout with Tailwind CSS",
                "Component-driven architecture with React",
                "Dark theme design system from scratch",
              ].map((item) => (
                <li key={item} className="flex items-start gap-3 text-[#6B7280] text-sm">
                  <span className="w-1.5 h-1.5 rounded-full bg-amber-500 mt-1.5 shrink-0"></span>
                  {item}
                </li>
              ))}
            </ul>
          </div>
        </div>

        {/* ── CTA ── */}
        <div className="bg-[#161616] border border-[#2A2A2A] rounded-3xl p-10 flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden">
          <div className="absolute -top-16 -right-16 w-64 h-64 bg-amber-500 rounded-full blur-3xl opacity-[0.04] pointer-events-none"></div>
          <div className="relative z-10">
            <p className="text-xs text-[#6B7280] uppercase tracking-widest mb-2">View Live</p>
            <h3 className="text-2xl font-black text-white">See it in action</h3>
            <p className="text-[#6B7280] text-sm mt-1">The site you're on right now!</p>
          </div>
          <div className="flex gap-4 relative z-10 shrink-0">
            <a
              href="https://github.com/gourav"
              target="_blank"
              rel="noopener noreferrer"
              className="px-6 py-3 bg-[#1C1C1C] border border-[#2A2A2A] text-white rounded-xl font-semibold hover:border-amber-500/40 hover:text-amber-400 transition-all duration-200 text-sm"
            >
              GitHub →
            </a>
            <a
              href="/"
              className="px-6 py-3 bg-amber-500 text-[#0F0F0F] rounded-xl font-bold hover:bg-amber-400 transition-all duration-200 text-sm shadow-lg shadow-amber-500/20"
            >
              Live Site →
            </a>
          </div>
        </div>

      </section>

      <Footer />
    </main>
  );
}
